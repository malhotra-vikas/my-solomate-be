    ...
    try {
        ...
        if (error) {
            if (error.code === "23505") {
                ...
                return NextResponse.json({ message: "Solo Mate is already in your SoloMates" }, { status: 200 });
            }
            console.error("Attach error:", error);
            return NextResponse.json({ error: "Failed to attach persona" }, { status: 500 });
        }

        return NextResponse.json({ message: "Solo Mate added successfully" }, { status: 200 });
    } catch (err) {
